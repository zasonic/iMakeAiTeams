...
- `sidecar.ts` — spawns and supervises the Python FastAPI sidecar
  (`../backend/server.py` in dev; `<app-resources>/backend/server[.exe]`
  when packaged, where `<app-resources>` is Electron's `process.resourcesPath`
  inside the installed app — distinct from the source `branding/` folder).
  Picks a random free port, generates a per-launch bearer token, and
  surfaces status events to the renderer.
