...
_SEED_PROMPTS: list[dict] = [
    ...
    },
    {
        "name": "prompt_judge",
        "category": "System",
        "description": "Judges A/B prompt comparisons.",
        "is_protected": True,
        "version_label": "1.0",
        "model_target": "claude",
        "text": (
            "You are a prompt evaluation judge. You receive two outputs "
            "generated from different prompts and evaluate them on the given "
            "criteria. Be objective and explain your reasoning.\n\n"
            "Return JSON: {\"winner\": \"A\"|\"B\"|\"tie\", "
            "\"scores\": {criterion: {\"A\": 1-5, \"B\": 1-5}}, "
            "\"overall_rationale\": str}"
        ),
        "notes": "For prompt A/B testing.",
    },
    {
...
def save_experiment(
    ...
    db.execute(
        ...
        "(id, prompt_a_version_id, prompt_b_version_id, test_input, output_a, output_b, "
        " judge_scores, judge_rationale, winner, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (exp_id, version_a_id, version_b_id, test_input, output_a, output_b,
         json.dumps(judge_scores), judge_rationale, winner, _now()),
    )
    db.commit()
    return exp_id
...
def list_experiments(limit: int = 20) -> list[dict]:
    rows = db.fetchall(
        "SELECT * FROM prompt_experiments ORDER BY created_at DESC LIMIT ?",
        (limit,),
    )
    return [dict(r) for r in rows]
...
def run_comparison(
    ...
    """
    Run an A/B comparison between two prompt versions using Claude as judge.
    Returns a dict with output_a, output_b, scores, winner, rationale.
    """
    a_row = db.fetchone(
        "SELECT pv.text, p.name FROM prompt_versions pv "
        "JOIN prompts p ON pv.prompt_id = p.id WHERE pv.id = ?",
        (version_a_id,),
    )
    b_row = db.fetchone(
        "SELECT pv.text, p.name FROM prompt_versions pv "
    ...
    sys_a, sys_b = a_row["text"], b_row["text"]
    output_a = claude_client.chat(sys_a, "", test_input, max_tokens=2048)
    output_b = claude_client.chat(sys_b, "", test_input, max_tokens=2048)

    judge_system = get_active_prompt("prompt_judge")
    crit_list = criteria or ["accuracy", "clarity", "task_adherence"]
    judge_input = (
        f"## Output A (from: {a_row['name']})\n{output_a}\n\n"
        f"## Output B (from: {b_row['name']})\n{output_b}\n\n"
        f"## Evaluation criteria: {', '.join(crit_list)}\n"
        f"## Test input was:\n{test_input}"
    )
    judge_raw = claude_client.chat(judge_system, "", judge_input, max_tokens=1024)
    raw = judge_raw.strip()
    if raw.startswith("```"):
        raw = raw.split("```")[1]
    ...
    try:
        ...
        scores = {"winner": "tie", "scores": {}, "overall_rationale": judge_raw}

    exp_id = save_experiment(
        version_a_id, version_b_id, test_input,
        output_a, output_b,
        scores.get("scores", {}),
        scores.get("overall_rationale", ""),
        scores.get("winner", "tie"),
    )
    return {
        ...
        "rationale": scores.get("overall_rationale", ""),
    }
