...
TOOL_SCHEMAS = [
    ...
    },
    {
        "name": "code_execute",
        "description": (
            "Execute Python code that can call other tools programmatically. "
            "Use this for processing large data, aggregating results, or any task where "
            "writing code is more efficient than multiple individual tool calls. "
            "The code has access to: tool(name, args_dict) to call any tool, "
            "json/re/math/collections modules, and a 'result' variable to return data. "
            "Set 'result = ...' to return structured data to the conversation."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string",
                    "description": "Python code to execute. Use tool('file_read', {'path': '...'}) to call tools.",
                },
            },
            "required": ["code"],
        },
...
class AgentLoop:
    ...
    def __init__(
        ...
        self._local   = local_client   # for architect mode (P10)
        self._gov     = governance     # governance engine (P9)
        self._stop    = False
    ...
    def run(self, task: str, conversation_id: str = "") -> str:
        ...
        # P10: Architect mode — get local model plan first (free)
        architect_plan = self._architect_plan(task, system)
        if architect_plan:
            # Prepend the plan to the task so Claude executes a pre-analyzed plan
            messages[0]["content"] = (
                f"## Task\n{task}\n\n"
                f"## Architect Plan (from local analysis)\n{architect_plan}\n\n"
                "Execute this plan step by step using the available tools. "
                "The plan was generated by a local model — verify each step is correct before executing."
            )
    ...
    # ── P10: Architect/Editor mode ───────────────────────────────────────

    def _architect_plan(self, task: str, system: str) -> str | None:
        """
        Use local model (free) to analyze the task and produce a plan
        before calling Claude for execution. Returns the plan text,
        or None if architect mode is disabled/unavailable.

        This is the Architect/Editor pattern from Aider: separate
        "what to change" (architect, local) from "how to change it"
        (editor, Claude).
        """
        if not self._local:
            return None
        ...
        architect_prompt = (
            "You are a code architect. Analyze this task and produce a precise, "
            "actionable plan. List the specific files to modify, what to change "
            "in each, and the order of operations. Be concise — no code, just the plan.\n\n"
            f"Task: {task}\n\n"
            "Respond with a numbered plan. Max 10 steps."
        )

        try:
            plan = self._local.chat(
                architect_prompt,
                f"Project root: {self._files.root}\n\nAnalyze and plan this task.",
                max_tokens=500,
            )
            if plan and not plan.startswith("[Local model"):
                self._emit("thinking", f"Architect plan:\n{plan}")
                return plan
        except Exception as exc:
            log.debug("Architect planning failed: %s", exc)
        return None
    ...
    def _handle_code_execute(self, args: dict):
        """Execute Python code with programmatic tool access."""
        from services.tools.code_runner import CodeRunner
        code = args.get("code", "")
        if not code:
            from services.tools.bash_tool import ToolResult
            return ToolResult(ok=False, error="No code provided.")

        # Build a tool dispatch function that routes to our handlers
        def _dispatch(name: str, tool_args: dict) -> str:
            handlers = {
                "file_read":  self._handle_file_read,
                "file_write": self._handle_file_write,
                "file_edit":  self._handle_file_edit,
            ...
            if hasattr(raw, "as_text"):
                return raw.as_text()
            return str(raw)

        runner = CodeRunner(_dispatch)
