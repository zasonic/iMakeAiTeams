...
def _render(rows: list[dict[str, Any]], methodology: dict[str, Any]) -> str:
    ...
    header = (
        ...
        "`.github/workflows/security-bench.yml` on every push to `main` and "
        "committed back into this repository, so the headline claim "
        "(\"7×–230× ASR reduction vs the Hackett et al. monolithic "
        "baseline\") is auditable from the commit history.\n\n"
        "## Results\n\n"
