...
v5.2 — Prompt caching extended to thinking paths:
  - extended_thinking_chat() and stream_extended_thinking_chat() now route
    the system prompt through _build_system_with_cache(). cache_control blocks
    are fully compatible with extended thinking (they operate at independent
    API layers). Repeated thinking calls with the same system prompt hit the
    cache at 0.1x the normal input-token rate, cutting extended-thinking input
    cost by 50–80% across consecutive turns within the 5-minute TTL window.
"""
