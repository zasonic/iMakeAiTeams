...
v5.2 — Token counting:
  - count_tokens() wraps the free /v1/messages/count_tokens endpoint (GA
    since late 2024). Returns 0 on any error so callers degrade safely.
    Useful for budget display before long requests and for workflow guards.
"""
...
class ClaudeClient:
    ...
    Handles:
      ...
      - Token counting (free, no inference cost)
    ...
    def count_tokens(
        ...
        """
        Return the estimated input token count for a request without making
        an inference call.

        Uses the free /v1/messages/count_tokens endpoint (GA since late 2024).
        Has its own independent rate limit — does not consume inference quota.
        Supports all content types: text, tools, images, PDFs, thinking blocks.

        Useful for:
          - Proactive budget display before the user sends a long message
          - Per-turn cost guards in multi-agent workflows
          - Test assertions on token counts without burning inference tokens

        Returns 0 on any error so all callers can safely ignore failures
        (e.g. when the API key is not yet configured).
        """
        kwargs: dict = {
            "model": self._model,
            "system": self._build_system_with_cache(system),
